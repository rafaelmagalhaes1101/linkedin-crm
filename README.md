...
Todos os nomes e valores são fictícios. Os e-mails iniciais usam `example.com`. Os botões de WhatsApp e e-mail ficam visíveis e desativados, sem links externos. O logotipo foi removido da apresentação e do painel.

O projeto não inclui SDK do Firebase, credenciais, configurações do banco real, autenticação ou endpoints de produção. Gráficos e fonte estão incluídos localmente. Não há analytics ou envio dos dados inseridos para um backend.
...
| `assets/` | Fonte Inter, Chart.js, ícone neutro e licenças |

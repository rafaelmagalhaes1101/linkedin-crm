...
// Ações de contato são demonstrativas: nunca abrem WhatsApp ou e-mail externos.
document.addEventListener('click', event => {
    const link = event.target.closest('a[href]');
    if (link && /^(mailto:|https:\/\/wa\.me\/)/i.test(link.getAttribute('href'))) {
        event.preventDefault();
        window.alert('Este contato é fictício. No CRM original, esta ação abre o WhatsApp ou o e-mail do cliente.');
    }
}, true);
